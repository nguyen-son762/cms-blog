  ...
  const updateLinkEditor = useCallback(() => {
    ...
    if (
      ...
      if (nativeSelection?.anchorNode === rootElement) {
        ...
        while ((inner as HTMLElement).firstElementChild != null) {
          inner = (inner as HTMLElement).firstElementChild;
        }
...
function Select({ onChange, className, options, value }: SelectProps) {
  return (
    <select className={className} onChange={e=>onChange} value={value}>
...
function BlockOptionsDropdownList({
  ...
  const formatBulletList = () => {
    if (blockType !== "ul") {
      editor.dispatchCommand(INSERT_UNORDERED_LIST_COMMAND, payload());
    } else {
      editor.dispatchCommand(REMOVE_LIST_COMMAND, payload());
    }
    setShowBlockOptionsDropDown(false);
  };
  ...
  const formatNumberedList = () => {
    if (blockType !== "ol") {
      editor.dispatchCommand(INSERT_ORDERED_LIST_COMMAND, payload());
    } else {
      editor.dispatchCommand(REMOVE_LIST_COMMAND, payload());
    }
    setShowBlockOptionsDropDown(false);
